...
async def save_document(document: DocumentModel, current_user: str = Depends(get_current_user)):
    try:
        ...
        # 添加用户ID和创建时间
        doc_data = document.model_dump()
        doc_data["userId"] = ObjectId(current_user)  # 将 userId 存储为 ObjectId
        ...
        return {
            ...
            "text": doc_data["text"],
            "userId": str(doc_data["userId"]),
            "createdAt": doc_data["createdAt"]
        }
    except Exception as e:
        print(f"保存文档错误: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_history(current_user: str = Depends(get_current_user)):
    try:
        ...
        # 获取当前用户的历史文档
        documents = []
        cursor = db.documents.find({"userId": ObjectId(current_user)}).sort("createdAt", -1)
