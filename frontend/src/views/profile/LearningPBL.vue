...
<div class="mb-8">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    <div v-for="badge in badgeStatus" 
         :key="badge.id" 
         class="badge-card relative group cursor-pointer rounded-lg overflow-hidden"
      ...
      <div class="p-4 h-full flex flex-col items-center justify-center text-center"
        ...
        <div class="text-4xl mb-2"
             :class="badge.status ? 'text-white' : 'text-gray-400'">
          <i :class="badge.icon"></i>
        </div>
        <h4 class="font-semibold"
            :class="badge.status ? 'text-white' : 'text-gray-500'">
          {{ badge.name }}
        </h4>
        <p class="text-sm mt-1"
           :class="badge.status ? 'text-white opacity-80' : 'text-gray-400'">
          {{ badge.description }}
        </p>
      </div>
      <!-- 完成标记 -->
      <div v-if="badge.status" 
    ...
    <!-- 排行榜 -->
    <div>
      <h3 class="text-xl font-semibold mb-4">成就排行榜</h3>
      <div class="bg-gray-50 rounded-lg p-4">
        <div v-if="leaderboard.length > 0">
  <div v-for="(user, index) in leaderboard" :key="index" 
       class="flex items-center p-3 mb-2 rounded-lg"
       :class="index === 0 ? 'bg-gradient-to-r from-yellow-100 to-yellow-200' : 
            ...
            <div class="flex-shrink-0 mr-4">
              <div :class="[
                ...
                index === 1 ? 'bg-gray-500' : 'bg-gray-400'
              ]">
                {{ index + 1 }}
              </div>
            </div>
    ...
    <div v-if="currentUser && user.userId === currentUser._id" class="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">
      我
    </div>
  </div>
        </div>
    ...
    >
      <div v-if="selectedBadge" class="text-center">
        <div :class="[
          ...
          <i :class="[selectedBadge.icon, 'text-4xl', isBadgeEarned(selectedBadge) ? 'text-white' : 'text-gray-400']"></i>
        </div>
